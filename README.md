...
#### `난이도 있는 문제를 풀어야 도움되지 않을까?`
------
### Java 풀이만 허용합니다!!!
------

## PR하는 법
1. 해당 레포를 fork해서 자신의 IDE에 clone하기
2. src/<내 이름>.week00/P1234_<문제이름> 으로 설정 후 문제 풀기  
_ex) src/홍길동.week01/P17831_대기업승범이네_
3. 자신의 레포에 commit & push 후 pull request하기
    1. **commit message 양식**  
    `<내이름> : <사이트이름> <문제번호> <문제이름>`  
    _ex) 홍길동 : 백준 1234 a+b_
    2. **PR 시 양식**  
    마지막으로 푼 `<내이름> : <N주차> <문제이름>` 으로 PR하기  
    _ex) 홍길동 : 2주차 카드게임_
